...
n = 15     # number of total sampling points
initial_n = 10     # number of initial sampling points
node_n = 100        # number of target points to evaluate in one sampling iteration
# iter_n = 10       # number of sampling points in each iteration
                    #NOTE: Could implement sample more points in one iteration
d = 3   # dimension
lb = [0.125,5.0,5.0]
ub = [1.,10.,10.]
...
for sample_iter in 1:(n-initial_n)
    ...
    function cv_error(new_sample_point_x)
        norm = 0
        poly_surrogate = LobachevskySurrogate(tempx, tempy, lb, ub) # Get Surrogate of current sampled points
    ...
    target_sample_xs = sample(node_n,lb,ub,SobolSample())       #NOTE: Try use other sampling methods other than Sobol Sample
...
# Find the MSE of 1000 tested points evaluated on Surrogates created using our cv sampling points
...
savefig("results/ndsampling_weldedbeam_MSE_$n p_$initial_n i_$node_n n.pdf")