...
for sample_iter in 1:(n-initial_n)
    ...
    clean_datatypef = x -> [i for i in x]       # Define a function that put a point in list datatype that can be input to other functions
    target_sample_xs = sample(node_n,lb,ub,SobolSample())   # Define node_n number of target sample points      #NOTE: Sobol Sample may collect same points as initial points
...
savefig("results/ndsampling_waterflow_MSE_$n p_$initial_n i_$node_n n.png")